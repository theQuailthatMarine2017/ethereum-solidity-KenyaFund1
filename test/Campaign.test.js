...
describe('Campaigns', () => {
  ...
  it('allows people to contribute and get marked as approvers', async () => {
  ...
  it('manager can make a payment request to approvers', async () => {
  ...
  it('proccesses request', async () => {
    ...
    let balance = await new web3.eth.getBalance(accounts[1]);
    balance = web3.utils.toWei(balance, 'ether');
    balance = parseFloat(balance);

    console.log("Balance from Account[1]: ", balance);
    assert(balance > 104);
  });
});
